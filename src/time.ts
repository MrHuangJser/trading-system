...
const RTH_START_SECONDS = 8 * SECONDS_PER_HOUR + 30 * SECONDS_PER_MINUTE; // 08:30:00
const RTH_END_SECONDS = 16 * SECONDS_PER_HOUR; // 16:00:00 (exclusive)
...
export function minuteKey(ts: ParsedTimestamp): string {
  return `${ts.date} ${ts.hour.toString().padStart(2, '0')}:${ts.minute
    .toString()
    .padStart(2, '0')}`;
}
